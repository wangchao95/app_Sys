...
//APP失去焦点验证的方法
    ...
    appSize.bind("blur", function () {
        ...
        verityfy(patrn,appSize,"App大小","app软件大小必须是大于1的数字");
    });

    //失去焦点验证的方法
    loadCount.bind("blur", function () {
        var patrn = /^[0-9]{0,}$/;
        verityfy(patrn,loadCount,"App下载次数","app软件下载次数必须是大于1数字");
    });
	 ...
	 * 修改软件信息
     */
    $(".form-group").on("click","#send,#againSend",function(){
        var status=$(this).val();
        appName.blur();
        rom.blur();
        viewLanguage.blur();
        ...
        if(appName.attr("validateStatus") != "true") {
            ...
            $("#appEdit").ajaxSubmit({
                type: 'post',
                url: "/sys/appInfo/editApp",
                data: {status: status},//请求参数
